...
dat <- data.frame(species = c("afarensis", "africanus", "habilis", "boisei", "rudolfensis", "ergaster", "sapiens"), pos = c(4, 3, 3, 4, 4, 3, 1), brain = c(438, 452, 612, 521, 752, 871, 1350), mass = c(37, 35.5, 34.5, 41.5, 55.5, 61, 53.5))

# Figure 7.2: brain volume versus body size
plot(brain ~ mass,
  data = dat, pch = 21, bg = "gray", xlab = "body mass (kg)",
  ylab = "brain volume (cc)", bty = "l", xlim = c(30, 70), ylim = c(
    200,
    1400
  )
)
...
res1 <- quap(alist(
  brain_std ~ dnorm(mu, exp(log_sigma)), mu <- a + b * mass_std,
...
res3 <- quap(alist(brain_std ~ dnorm(mu, exp(log_sigma)), mu <- a + b[1] *
  ...
  dnorm(0, 10), log_sigma ~ dnorm(0, 1)), data = dat, start = list(b = rep(
  0,
  3
)))
res4 <- quap(
  alist(
    brain_std ~ dnorm(mu, exp(log_sigma)), mu <- a + b[1] *
...
res5 <- quap(alist(brain_std ~ dnorm(mu, exp(log_sigma)), mu <- a + b[1] *
  mass_std + b[2] * mass_std^2 + b[3] * mass_std^3 + b[4] * mass_std^4 +
  b[5] * mass_std^5, a ~ dnorm(0.5, 1), b ~ dnorm(0, 10), log_sigma ~ dnorm(
  0,
  1
)), data = dat, start = list(b = rep(0, 5)))
res6 <- quap(alist(brain_std ~ dnorm(mu, exp(log_sigma)), mu <- a + b[1] *
  mass_std + b[2] * mass_std^2 + b[3] * mass_std^3 + b[4] * mass_std^4 +
  b[5] * mass_std^5 + b[6] * mass_std^6, a ~ dnorm(0.5, 1), b ~ dnorm(
  0,
  10
...
plot(brain_std ~ mass_std,
  data = dat, pch = 21, bg = "gray", xlab = "standardized body mass (kg)",
...
for (i in 1:6) {
  ...
  plot(brain_std ~ mass_std,
    data = dat, pch = 21, bg = "gray", xlab = "body mass (kg)",
    ylab = "brain volume (cc)", bty = "l", xlim = range(xs_std), ylim = c(min(
      ci,
      brain_std
    ), max(ci, brain_std)), xaxt = "n", yaxt = "n"
  )
...
plot(brain_std ~ mass_std,
  data = dat, pch = 21, bg = "gray", xlab = "body mass (kg)",
  ylab = "brain volume (cc)", bty = "l", xlim = range(xs_std), ylim = c(
    0,
    1400
  ) / max(dat$brain), xaxt = "n", yaxt = "n"
)
axis(side = 1, at = xs_std, labels = xs)
axis(side = 2, at = ys_std, labels = ys)
...
invisible(lapply(1:7, function(i) {
  tmp <- quap(alist(brain_std ~ dnorm(mu, exp(log_sigma)), mu <- a + b *
    mass_std, a ~ dnorm(0.5, 1), b ~ dnorm(0, 10), log_sigma ~ dnorm(
    0,
    1
  )), data = dat[-i, ])
...
invisible(lapply(1:7, function(i) {
  tmp <- quap(alist(
    brain_std ~ dnorm(mu, exp(log_sigma)), mu <- a + b[1] *
      mass_std + b[2] * mass_std^2 + b[3] * mass_std^3 + b[4] * mass_std^4,
    a ~ dnorm(0.5, 1), b ~ dnorm(0, 10), log_sigma ~ dnorm(0, 1)
  ), data = dat[-i, ], start = list(b = rep(0, 4)))
  l <- link(tmp, data = list(mass_std = mass_seq))
  mu <- apply(l, 2, mean)
  lines(mass_seq, mu, lwd = 3, col = adjustcolor("black", alpha.f = 0.4))
}))
...
sapply(1:7, function(i) {
  log(mean(apply(post, 1, function(par) {
    dnorm(dat$brain_std[i],
      mean = par[1] + par[2] * dat$mass_std[i],
      sd = exp(par[3])
    )
  })))
})
...
fitmodels <- function(dat) {
  ...
  res2 <- quap(alist(y ~ dnorm(mu, 1), mu <- a + b[1] * X1, a ~ dnorm(
    0,
    1
  ), b ~ dnorm(0, 10)), data = dat, start = list(b = rep(0, 1)))
  res3 <- quap(alist(
    y ~ dnorm(mu, 1), mu <- a + b[1] * X1 + b[2] * X2,
    a ~ dnorm(0, 1), b ~ dnorm(0, 10)
  ), data = dat, start = list(b = rep(
    0,
    2
  )))
  res4 <- quap(alist(y ~ dnorm(mu, 1), mu <- a + b[1] * X1 + b[2] * X2 +
    b[3] * X3, a ~ dnorm(0, 1), b ~ dnorm(0, 10)), data = dat, start = list(b = rep(
    0,
    3
  )))
  res5 <- quap(
    alist(y ~ dnorm(mu, 1), mu <- a + b[1] * X1 + b[2] * X2 +
      b[3] * X3 + b[4] * X4, a ~ dnorm(0, 1), b ~ dnorm(0, 10)),
    data = dat,
    start = list(b = rep(0, 4))
  )
...
# n=20
plot(NA, xlim = c(0.8, 5.2), ylim = c(min(dev.train.lo, dev.test.lo), max(
  dev.train.hi,
  dev.test.hi
)), xlab = "number of parameters", ylab = "deviance", main = paste(
  "N =",
  n
))
