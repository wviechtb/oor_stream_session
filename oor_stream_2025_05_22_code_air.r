...
plot(
  ...
  pch = 21,
  bg = "gray",
  xlab = "standardized body mass (kg)",
...
for (i in 1:6) {
  ...
  plot(
    ...
    bty = "l",
    xlim = range(xs_std),
    ylim = c(min(ci, brain_std), max(ci, brain_std)),
    xaxt = "n",
    yaxt = "n"
  )
  ...
  shade(ci, mass_seq)
  if (i == 6) abline(h = 0, lty = "dashed")
  mtext(
    paste0(
      "Polynomial: ",
      i,
      " (R^2 = ",
      round(R2_is_bad(res[[i]]), digits = 2),
      ")"
    ),
    line = 2,
    cex = 0.8
...
plot(
  NA,
  xlim = c(0.8, 5.2),
  ylim = c(min(dev.train.lo, dev.test.lo), max(dev.train.hi, dev.test.hi)),
